    ...
    (name : names) ->
      let scopeGraph' =
            maybe
              ...
              (const (insertEdge edge name currentAddress g))
              (lookupScope name g)
       in addImportEdge edge names name scopeGraph'

addImportHole :: Ord scopeAddress => EdgeLabel -> scopeAddress -> scopeAddress -> ScopeGraph scopeAddress -> ScopeGraph scopeAddress
addImportHole edge name currentAddress g =
  let scopeGraph' = newScope name mempty g
   in insertEdges (NonEmpty.fromList [Void, edge]) name currentAddress scopeGraph'
